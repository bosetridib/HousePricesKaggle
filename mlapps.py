...
X_test.drop(columns=['Street','Utilities','Condition2'], inplace=True)
print(X_train.select_dtypes(['category']).describe().iloc[:,10:20])
X_train.drop(columns=['RoofMatl'], inplace=True)
X_test.drop(columns=['RoofMatl'], inplace=True)
...
X_train_encoded = X_train.select_dtypes(['int64', 'float64'])
for i in X_train.select_dtypes('category').columns:
    X_train_encoded = pd.concat(
        [X_train_encoded, pd.get_dummies(X_train[i], prefix=i).astype('int')],
        axis=1
    )

X_test_encoded = test.select_dtypes(['int64','float64'])
for i in X_test.select_dtypes('category').columns:
    X_test_encoded = pd.concat(
        [X_test_encoded, pd.get_dummies(X_test[i], prefix=i).astype('int')],
        axis=1
    )

# Remove different columns in the one-hot encoded train and test datasets

for i,j in zip(X_train_encoded.columns, X_test_encoded.columns):
    if i not in X_test_encoded.columns:
        X_train_encoded.drop(columns=i, inplace=True)
    if j not in X_train_encoded.columns:
        X_test_encoded.drop(columns=j, inplace=True)

from sklearn.ensemble import GradientBoostingRegressor
rmse = []

for i,j,k in [(i,j,k) for i in range(100,3100,500) for j in range(1,21,4) for k in range(3,21,6)]:
    
    gradient_boost_model = GradientBoostingRegressor(n_estimators=i, max_depth=j, min_samples_leaf=k)
    gradient_boost_model.fit(X_train_encoded, Y_train.values)
    
    Y_train_est = gradient_boost_model.predict(X_train_encoded)
...
del rmse,r,i,j,k
gradient_boost_model.fit(X_train_encoded, Y_train.values)
Y_test = gradient_boost_model.predict(X_test_encoded)
